...
using UnityEngine;
    ...
    class AlgDijkstra : MonoBehaviour
    {
        ...
        public static void Dijkstra(GrafoMA grafo, int source)
        {
            ...
            for (int i = 0; i < verticesCount; i++)
            {
            if (i < nodos1.Length && i < nodos2.Length)
            {
                if (nodos1[i] != -1)
                {
            ...
            {
                Debug.LogError($"Index out of bounds: i={i}, nodos1.Length={nodos1.Length}, nodos2.Length={nodos2.Length}");
            }
